...
function FeatureRevealText({ title, description }: { title: string; description: string }) {
  ...
  return (
    <div ref={ref} className="space-y-3">
      ...
      <div className="relative">
        <p className="text-[17px] leading-[1.7] text-neutral-300">{description}</p>
        <motion.p
...
function MobileFeatureCarousel({ features }: { features: { title: string; description: string; color: string }[] }) {
    ...
    <div className="md:hidden">
      <div className="relative px-6">
        ...
        <div className="absolute bottom-4 right-4 flex gap-2 z-10 pr-4">
          <button onClick={() => goTo(activeIndex - 1)} className="w-12 h-12 rounded-[16px] bg-white/90 flex items-center justify-center text-neutral-900 text-lg" aria-label="Prev">‹</button>
          <button onClick={() => goTo(activeIndex + 1)} className="w-12 h-12 rounded-[16px] bg-white/90 flex items-center justify-center text-neutral-900 text-lg" aria-label="Next">›</button>
        </div>
      </div>
      <div className="px-6 mt-5">
        ...
        <div className="flex gap-1.5 mt-4">
          {features.map((_, i) => (
            <div key={i} className={`h-[3px] rounded-full transition-all duration-300 ${i === activeIndex ? "w-8 bg-neutral-900" : "w-4 bg-neutral-200"}`} />
          ))}
        </div>
      </div>
    </div>
...
export default function FeatureShowcase() {
  ...
  const t = useT();
  const features = t.featureShowcase.features;
      ...
      <div className="hidden md:flex">
        <div className="w-full md:w-1/2 shrink-0" style={{ padding: "0 60px" }}>
          {features.map((feature, i) => (
            ...
            >
              <FeatureRevealText title={feature.title} description={feature.description} />
            </div>
          ))}
        </div>
          ...
          <div className="relative h-full p-5">
            <div className="absolute inset-5 overflow-hidden rounded-tr-[32px] rounded-bl-[32px]">
              <div className="relative w-full h-full">
                ...
                <svg className="absolute top-0 left-0 w-full z-10" viewBox="0 0 1440 36" preserveAspectRatio="none" fill="white" xmlns="http://www.w3.org/2000/svg" style={{ height: "36px" }}>
                  <path d="M0 0H1440V4C1440 21.7 1425.7 36 1408 36H820C780 36 760 20 740 8C720 0 710 0 680 0H32C14.3 0 0 14.3 0 32V0Z" />
                </svg>
                <svg className="absolute bottom-0 left-0 w-full z-10" viewBox="0 0 1440 36" preserveAspectRatio="none" fill="white" xmlns="http://www.w3.org/2000/svg" style={{ height: "36px" }}>
