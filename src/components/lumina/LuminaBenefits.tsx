...
export default function LuminaBenefits() {
  ...
  const t = useT();
  const benefits = t.lumina.benefits;
  ...
  return (
    <section ref={sectionRef} className="relative bg-[#0a2e2e] py-24 md:py-32 px-4 md:px-6 overflow-hidden">
      <motion.div style={{ y: dotY }} className="absolute inset-[-10%] pointer-events-none">
        <div className="w-full h-full opacity-[0.15]" style={{ backgroundImage: "radial-gradient(circle, #21beba 1px, transparent 1px)", backgroundSize: "32px 32px" }} />
      </motion.div>
      <div className="relative max-w-[1080px] mx-auto">
        <motion.div
          ...
          style={{ boxShadow: "0 0 30px rgba(33,190,186,0.06), inset 0 0 30px rgba(33,190,186,0.03)" }}
        >
          <div className="grid grid-cols-1 md:grid-cols-3 gap-x-0 gap-y-8">
            {benefits.slice(0, 3).map((b, i) => (
              <div key={i} className={`px-0 md:px-6 ${i < 2 ? "md:border-r md:border-[#21beba]/15" : ""}`} style={i < 2 ? { borderRightStyle: "solid" } : undefined}>
                <h3 className="text-[17px] font-bold text-[#21beba] leading-snug mb-3">{b.title}</h3>
                <p className="text-[14px] leading-[1.7] text-white/50">{b.desc}</p>
              </div>
            ))}
          </div>
          ...
          <div className="px-0 md:px-6">
            <h3 className="text-[17px] font-bold text-[#21beba] leading-snug mb-3">{benefits[6].title}</h3>
            <p className="text-[14px] leading-[1.7] text-white/50">{benefits[6].desc}</p>
          </div>
